...
use crate::machinst::{MachInsts, MachRegConstraints};
...
pub struct Function {
    ...
    /// Machine-instruction encodings, for the new MachInst-based backend.
    pub machinsts: Option<Box<dyn MachInsts>>,

    /// Machine register constraints, generated by the new MachInst-based backend.
    pub reg_constraints: Option<MachRegConstraints>,
...
impl Function {
    ...
    /// Returns the MachInsts container for the lowered machine instructions of this function.
    pub fn machinsts(&self) -> Option<&dyn MachInsts> {
        self.machinsts.as_ref().map(|m| &**m)
    }
}
